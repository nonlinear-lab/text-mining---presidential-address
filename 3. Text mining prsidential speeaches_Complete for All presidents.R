# ============================================================
# TEXT MINING + TOPIC-SENTIMENT CORRELATION:
# US Presidents' Inaugural Speeches
# ============================================================

# --- Load libraries ---
library(tidyverse)
library(tidytext)
library(tm)
library(wordcloud)
library(SnowballC)
library(ggplot2)
library(textdata)
library(topicmodels)
library(widyr)
library(ggraph)
library(igraph)
library(RColorBrewer)
library(plotly)
library(tidyr)

# ------------------------------------------------------------
# Step 1: Load all speeches automatically
# ------------------------------------------------------------
# setwd("G:/My Drive/OlaOluwas/Program Codes/R code_Text mining_Inaugural Presidential Speeches/US presidents inaugural speeches")
# Set the folder where your files are located
setwd("C:/R/speech") 



files <- list.files(pattern = "*.txt")

speeches <- map_df(files, function(f) {
  text <- readLines(f, encoding = "UTF-8")
  tibble(president = tools::file_path_sans_ext(f),
         text = paste(text, collapse = " "))
})

# ------------------------------------------------------------
# Step 2: Tokenize and clean
# ------------------------------------------------------------
data("stop_words")

tidy_speeches <- speeches %>%
  unnest_tokens(word, text) %>%
  mutate(word = str_replace_all(word, "[^a-zA-Z]", "")) %>%
  filter(word != "", nchar(word) > 2) %>%
  anti_join(stop_words, by = "word")

# ------------------------------------------------------------
# Step 3: Word Frequency and Wordcloud
# ------------------------------------------------------------
word_freq <- tidy_speeches %>%
  count(president, word, sort = TRUE)

# Wordcloud for all speeches
set.seed(123)
wordcloud(words = word_freq$word,
          freq = word_freq$n,
          max.words = 100,
          random.order = FALSE,
          colors = brewer.pal(8, "Dark2"))

write.csv(word_freq$word, "word_freq.csv")
write.csv(word_freq, "freq.csv")
# ------------------------------------------------------------
# Step 4: Topic Modeling (LDA)
# ------------------------------------------------------------
dtm <- tidy_speeches %>%
  count(president, word) %>%
  cast_dtm(document = president, term = word, value = n)

set.seed(123)
lda <- LDA(dtm, k = 5, method = "VEM")

topics_tidy <- tidy(lda, matrix = "beta")      # term-topic probabilities
gamma_tidy <- tidy(lda, matrix = "gamma")      # document-topic probabilities

# Top terms per topic
top_terms <- topics_tidy %>%
  group_by(topic) %>%
  slice_max(beta, n = 10) %>%
  ungroup()

# Plot top terms (interactive)
p_topic <- ggplot(top_terms, aes(x = reorder_within(term, beta, topic),
                                 y = beta, fill = factor(topic))) +
  geom_col(show.legend = FALSE) +
  facet_wrap(~ topic, scales = "free_y") +
  coord_flip() +
  scale_x_reordered() +
  labs(title = "Top Terms per Topic (LDA)",
       x = "Term", y = "Probability") +
  theme_minimal()
plotly::ggplotly(p_topic)

# ------------------------------------------------------------
# Step 5: Sentiment Analysis (NRC)
# ------------------------------------------------------------
nrc <- get_sentiments("nrc")

tidy_sent <- tidy_speeches %>%
  inner_join(nrc, by = "word")

sentiment_summary <- tidy_sent %>%
  count(president, sentiment, sort = TRUE)

# Plot sentiment distribution per president
p_sent <- ggplot(sentiment_summary, aes(x = reorder_within(sentiment, n, president),
                                        y = n, fill = sentiment)) +
  geom_col(show.legend = FALSE) +
  facet_wrap(~ president, scales = "free_y") +
  coord_flip() +
  scale_x_reordered() +
  labs(title = "Sentiment Distribution per President (NRC)",
       x = "Sentiment", y = "Word Count") +
  theme_minimal()
plotly::ggplotly(p_sent)

# ------------------------------------------------------------
# Step 6: Topic-Sentiment Correlation
# ------------------------------------------------------------
# 1. Join top topic words with sentiment
topic_sent <- top_terms %>%
  inner_join(nrc, by = c("term" = "word")) %>%
  count(topic, sentiment, sort = TRUE)

# 2. Visualize topic-sentiment relationship
p_topic_sent <- ggplot(topic_sent, aes(x = sentiment, y = n, fill = sentiment)) +
  geom_col(show.legend = FALSE) +
  facet_wrap(~ topic, scales = "free_y") +
  coord_flip() +
  labs(title = "Sentiment Distribution per Topic",
       x = "Sentiment", y = "Frequency") +
  theme_minimal()
plotly::ggplotly(p_topic_sent)

# 3. Optional: calculate correlation between topic probability and sentiment
# Merge gamma_tidy (document-topic) with sentiment counts per document (president)
gamma_sent <- gamma_tidy %>%
  rename(president = document) %>%
  left_join(tidy_sent %>% count(president, sentiment), by = "president") %>%
  group_by(topic, sentiment) %>%
  summarise(avg_gamma = mean(gamma, na.rm = TRUE),
            total_sent = sum(n, na.rm = TRUE),
            .groups = "drop") %>%
  mutate(topic_sent_score = avg_gamma * total_sent)

# Heatmap of topic-sentiment correlation
p_heat <- ggplot(gamma_sent, aes(x = factor(topic), y = sentiment, fill = topic_sent_score)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "lightyellow", high = "darkred") +
  labs(title = "Topic-Sentiment Correlation Heatmap",
       x = "Topic", y = "Sentiment", fill = "Score") +
  theme_minimal()
plotly::ggplotly(p_heat)


# ------------------------------------------------------------
# Step 8: Bigram Analysis (Context)
# ------------------------------------------------------------
# Create bigrams
speeches_bigrams <- speeches %>%
  unnest_tokens(bigram, text, token = "ngrams", n = 2)

# Separate bigrams to remove stop words
bigrams_separated <- speeches_bigrams %>%
  separate(bigram, c("word1", "word2"), sep = " ")

# Filter out stop words from both parts of the bigram
bigrams_filtered <- bigrams_separated %>%
  filter(!word1 %in% stop_words$word) %>%
  filter(!word2 %in% stop_words$word) %>%
  filter(!str_detect(word1, "\\d"), !str_detect(word2, "\\d")) # Remove numbers

# Count new bigram frequencies
bigram_counts <- bigrams_filtered %>%
  count(word1, word2, sort = TRUE)

# Unite them back for visualization
bigrams_united <- bigrams_filtered %>%
  unite(bigram, word1, word2, sep = " ")

# Plot Top 20 Bigrams
p_bigram <- bigrams_united %>%
  count(bigram, sort = TRUE) %>%
  slice_max(n, n = 20) %>%
  mutate(bigram = reorder(bigram, n)) %>%
  ggplot(aes(n, bigram, fill = n)) +
  geom_col(show.legend = FALSE) +
  labs(title = "Top 20 Bigrams (Two-Word Phrases)", x = "Frequency", y = NULL) +
  theme_minimal()

plotly::ggplotly(p_bigram)


# ------------------------------------------------------------
# Step 9: Network Graph of Co-occurring Words
# ------------------------------------------------------------
# Create the graph object from bigram counts (filtering for noise)
bigram_graph <- bigram_counts %>%
  filter(n > 5) %>% # Only show pairs appearing more than 5 times
  graph_from_data_frame()

set.seed(2023)
ggraph(bigram_graph, layout = "fr") +
  geom_edge_link(aes(edge_alpha = n), show.legend = FALSE,
                 arrow = grid::arrow(type = "closed", length = unit(.1, "inches")), 
                 end_cap = circle(.07, 'inches')) +
  geom_node_point(color = "lightblue", size = 5) +
  geom_node_text(aes(label = name), vjust = 1, hjust = 1) +
  theme_void() +
  labs(title = "Network of Word Co-occurrences")

# ------------------------------------------------------------
# Step 10: TF-IDF (Distinctive Words per President)
# ------------------------------------------------------------
president_tfidf <- tidy_speeches %>%
  count(president, word, sort = TRUE) %>%
  bind_tf_idf(word, president, n)

# Visualize the most distinctive words for specific presidents
# (Adjust the filter to select specific presidents you want to compare)
p_tfidf <- president_tfidf %>%
  group_by(president) %>%
  slice_max(tf_idf, n = 10) %>%
  ungroup() %>%
  # Example: Let's filter for top 4 presidents by distinctiveness or specific names
  # Replace distinct(president) with specific names if desired
  filter(president %in% c(unique(president)[1:4])) %>% 
  ggplot(aes(tf_idf, reorder_within(word, tf_idf, president), fill = president)) +
  geom_col(show.legend = FALSE) +
  facet_wrap(~president, scales = "free") +
  scale_y_reordered() +
  labs(title = "Most Distinctive Words (TF-IDF)",
       x = "TF-IDF Score", y = NULL) +
  theme_minimal()

# ------------------------------------------------------------
# Step 11: Lexical Diversity & Complexity
# ------------------------------------------------------------

# Calculate sentence length (approximate based on periods)
sentence_complexity <- speeches %>%
  mutate(sentences = str_count(text, "[.?!]"),
         word_count = str_count(text, "\\w+")) %>%
  mutate(avg_sentence_length = word_count / sentences)

# Calculate Lexical Diversity (Type-Token Ratio)
# Unique words / Total words
lexical_diversity <- tidy_speeches %>%
  group_by(president) %>%
  summarise(unique_words = n_distinct(word),
            total_words = n(),
            lexical_diversity = unique_words / total_words) %>%
  inner_join(sentence_complexity, by = "president")

# Plot Complexity vs Diversity
p_complexity <- ggplot(lexical_diversity, 
                       aes(x = avg_sentence_length, y = lexical_diversity, label = president)) +
  geom_point(color = "darkblue", size = 3, alpha = 0.7) +
  geom_text(vjust = 1.5, check_overlap = TRUE, size = 3) +
  labs(title = "Speech Complexity: Diversity vs. Sentence Length",
       x = "Avg Words per Sentence (Length)",
       y = "Lexical Diversity (Unique Word Usage)") +
  theme_minimal()

plotly::ggplotly(p_complexity)
plotly::ggplotly(p_tfidf)


# ------------------------------------------------------------
# Step 12: Sentiment Narrative Arc (Start to Finish)
# ------------------------------------------------------------
narrative_arc <- tidy_speeches %>%
  inner_join(get_sentiments("bing"), by = "word") %>%
  count(president, index = 1:n() %/% 40, sentiment) %>% # Groups every 50 words
  spread(sentiment, n, fill = 0) %>%
  mutate(sentiment_score = positive - negative) %>%
  group_by(president) %>%
  mutate(pct_progress = row_number() / n() * 100) %>% # Normalize length to 0-100%
  ungroup()

# Plot the flow for a few specific presidents (to avoid overcrowding)
# You can change the names in the filter below
target_presidents <- unique(speeches$president)[1:5] # Selects first 5 alphabetically

p_flow <- narrative_arc %>%
  filter(president %in% target_presidents) %>% 
  ggplot(aes(x = pct_progress, y = sentiment_score, color = president)) +
  geom_smooth(method = "loess", se = FALSE) + # Smooth trend line
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
  labs(title = "Emotional Arc of Speeches",
       subtitle = "Smoothed Sentiment from Start (0%) to End (100%)",
       x = "Progress through Speech (%)",
       y = "Sentiment Score (Pos - Neg)") +
  theme_minimal()

plotly::ggplotly(p_flow)


# ------------------------------------------------------------
# Step 13: Hierarchical Clustering (Who sounds like whom?)
# ------------------------------------------------------------
# 1. Normalize the Document-Term Matrix (TF-IDF weighting generally better for clustering)
dtm_tfidf <- DocumentTermMatrix(Corpus(VectorSource(speeches$text)),
                                control = list(weighting = weightTfIdf))

# Remove sparse terms (words that appear very rarely) to reduce noise
dtm_sparse <- removeSparseTerms(dtm_tfidf, 0.95)

# 2. Calculate Distance Matrix
dist_matrix <- dist(as.matrix(dtm_sparse), method = "euclidean")

# 3. Hierarchical Clustering
fit <- hclust(dist_matrix, method = "ward.D2")

# 4. Plot Dendrogram
# We use the 'ggdendro' package or base plot. Base plot is simpler here.
plot(fit, 
     main = "Cluster Dendrogram of Presidents' Speeches",
     xlab = "President", 
     sub = "",
     cex = 0.8) # Adjust cex for text size

# Highlight groups with rectangles (e.g., 4 clusters)
rect.hclust(fit, k = 4, border = "red")


# ------------------------------------------------------------
# Step 16: Phrase Nets (Visualizing "X of Y" patterns)
# ------------------------------------------------------------
# We look for words separated by "of", "and", or "to"
phrase_net_data <- speeches %>%
  unnest_tokens(bigram, text, token = "ngrams", n = 2) %>%
  separate(bigram, c("word1", "word2"), sep = " ") %>%
  filter(word1 %in% c("of", "and", "to") | word2 %in% c("of", "and", "to")) %>%
  count(word1, word2, sort = TRUE) %>%
  filter(n > 10) %>% # Filter for frequent connections
  graph_from_data_frame()

# Visualize the connections
set.seed(2024)
ggraph(phrase_net_data, layout = "fr") +
  geom_edge_link(aes(edge_alpha = n, edge_width = n), edge_colour = "darkred",
                 arrow = grid::arrow(type = "closed", length = unit(2, "mm")), 
                 end_cap = circle(3, 'mm')) +
  geom_node_point(size = 4, color = "black") +
  geom_node_text(aes(label = name), vjust = 1.5, repel = TRUE) +
  theme_void() +
  labs(title = "Phrase Net: How words are linked",
       subtitle = "Visualizing connectors like 'of', 'and', 'to'")


# ------------------------------------------------------------
# ------------------------------------------------------------
# Step 18: Sentiment Volatility (Emotional "Rollercoaster" Index)
# ------------------------------------------------------------
volatility <- tidy_speeches %>%
  inner_join(get_sentiments("afinn"), by = "word") %>% # AFINN has numeric scores (-5 to +5)
  group_by(president) %>%
  summarise(
    sentiment_mean = mean(value),
    sentiment_volatility = sd(value), # Standard Deviation = Volatility
    word_count = n()
  ) %>%
  filter(word_count > 50) # Remove very short texts

# Plot Volatility
p_volatility <- ggplot(volatility, aes(x = sentiment_mean, y = sentiment_volatility, label = president)) +
  geom_point(color = "purple", size = 3, alpha = 0.7) +
  geom_text(vjust = 1.5, check_overlap = TRUE, size = 3) +
  labs(title = "Emotional Stability of Presidents",
       x = "Average Sentiment (Positive/Negative)",
       y = "Volatility (Emotional Swings)") +
  theme_minimal()

plotly::ggplotly(p_volatility)




# ------------------------------------------------------------
# ------------------------------------------------------------


# Step 7: Export results
# ------------------------------------------------------------
write.csv(top_terms, "LDA_TopTerms.csv", row.names = FALSE)
write.csv(sentiment_summary, "Sentiment_NRC.csv", row.names = FALSE)
write.csv(topic_sent, "Topic_Sentiment.csv", row.names = FALSE)
write.csv(gamma_sent, "Topic_Sentiment_Correlation.csv", row.names = FALSE)
